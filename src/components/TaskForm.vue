<template>
  <form class="task-form" @submit.prevent="handleSubmit">
    <div class="task-row">
      ...
      <button type="submit" class="task-button" :disabled="uploading">{{ editingTask ? 'Alterar' : 'Adicionar' }}</button>
      <button v-if="editingTask" type="button" class="task-button-cancel" @click="handleCancel">Cancelar</button>
    </div>
    <div class="image-section">
      <img v-if="previewUrl || editingTask?.img_url" :src="previewUrl || editingTask?.img_url" class="image-preview" alt="Imagem da tarefa" />
      <label class="image-label" :class="{ disabled: uploading }">
        <span v-if="uploading" class="upload-status">Enviando...</span>
        <span v-else>{{ previewUrl || editingTask?.img_url ? 'Trocar imagem' : 'Adicionar imagem' }}</span>
        <input type="file" accept="image/jpeg,image/png" capture="environment" class="image-input" :disabled="uploading" @change="handleImageChange" />
      </label>
      <p class="image-help">Em celular, o botão pode abrir a câmera. Em notebook, abre o seletor de arquivos.</p>
    </div>
    <div class="location-section">
      <button type="button" class="location-button" :disabled="loadingLocation" @click="capturarLocalizacao">
        {{ loadingLocation ? 'Obtendo localização...' : 'Capturar localização' }}
      </button>
      <button v-if="location" type="button" class="location-clear" @click="clearLocation">Remover</button>
...
async function capturarLocalizacao() {
  ...
  if (!result) return
  try {
    const address = await geocodingApi.reverse(result.latitude, result.longitude)
    if (address?.label) {
